...
void * receiver_input_generator_fn(void * arg)
{
	...
	xcwcp_receiver->rec_tester.helper_gen = gen;
	cw_tq_register_low_level_callback_internal(xcwcp_receiver->rec_tester.helper_gen->tq, low_tone_queue_callback, &xcwcp_receiver->rec_tester, 5);
	...
	for (int i = 0; i < 5; i++) {
		...
		if ('\0' == c) {
			...
			cw_gen_enqueue_character(xcwcp_receiver->rec_tester.helper_gen, c);
	...
	cw_rec_tester_evaluate_receive_correctness(&xcwcp_receiver->rec_tester, &test_passes);
