...
* SKIPPED Review descriptions of changes
...
acerion 2023.11.17: I'm marking this item as SKIPPED because I have reviewed
only changes made since 3.6.1.
...
acerion 2023.11.17: skipped: localization is not a priority for me now.
...
* DONE Make final test using code from release archive
  ...
  4. Run unixcw's programs from the location in which you have built the
     package. Check that programs start correctly, that all basic functions
     are running as expected, that there is no crash or error that would
     discourage user who is trying to use unixcw.
...
* DONE Make final sanity check test using code from repository
  ...
  correctly here. You have already did this in previous point.

* SKIPPED Update statuses of issues in issue tracker
...
2023.11.18 acerion: skipped on purpose. I want to look at the tickets in next
release.

* STARTED Publish your release archive with your program
...
Use git's annotated tag:
   ...
   e.g.
   git tag -a v.3.6.1 -m "Tagging release v3.6.1"
